...
class MpcFlyWind:
    def __init__(self, v_para, v_perp, phi, w, zeta, x0=None, dt=0.01, n_horizon=20, r_weight=1e-9):
        """ Use Model Predictive Control (MPC) to control simulated fly-wind trajectories.
            ...
            Inputs:
                ...
                v_para: time series for parallel velocity
                v_perp: time series for perpendicular velocity
                phi: time series for heading
        """
        ...
        self.x0 = {'v_para': self.v_para[0],
                   ...
                   'zeta': self.zeta[0],
                   'I': 0.01, 'm': 1.0,
                   'C_para': 1.0, 'C_perp': 1.0, 'C_phi': 1.0, 'd': 1.0,
                   'km1': 1.0, 'km2': 0.0, 'km3': 1.0, 'km4': 1.0,  # motor calibration parameters
                   'ks1': 1.0, 'ks2': 1.0, 'ks3': 0.0, 'ks4': 1.0, 'ks5': 0.0, 'ks6': 1.0,
                   'ks7': 0.0}  # sensor calibration parameters
        ...
        self.system = FlyWindDynamics(control_mode='open_loop', update_every_step=True, output_mode=output_mode)
        self.sim_data = {}
    ...
    def plot_setpoint_tracking(self, size=7, lw=2):
        """ Plot closed-loop MPC states trajectories vs the desired set-point.
        """
        fig, ax = plt.subplots(2, 1, figsize=(size, 1.2 * size), dpi=100)
        ...
        ax[1].plot(self.tsim, self.u_mpc[:, 2], '-', label='u_phi', lw=lw)
